"""
DATOS DE LA BASE DE DATOS tornillos.db, TABLA ISO_DIN_13_extendido
...
class DatosISODIN13Ext:
    def __init__(self) -> None:
        """
        Inicializa la clase DatosISODIN13Ext, que se encarga de proporcionar acceso a los datos de la base de datos
        'tornillos.db', específicamente la tabla 'ISO_DIN_13_extendido'. Contiene métricas de M1 a M210.
        """
        # Ruta a la base de datos de tornillos
        self.ruta_datos = f'{Paths.data}\\'
        self.tornillos_db = 'tornillos.db'
        self.tabla = 'ISO_DIN_13_extendido'

        # Clase que gestiona la conexión a SQLite y la conversión a DataFrame
        self.sql_pd = SQLitePandasDF

        # Cache para almacenar el DataFrame y evitar múltiples lecturas de la base de datos
        self._df_cache = None

    def _load_dataframe(self) -> pd.DataFrame:
        """
        Carga el DataFrame desde la base de datos SQLite y lo cachea para futuros accesos.

        :return: DataFrame con los datos de la tabla.
        """
        if self._df_cache is None:  # Solo cargar los datos si no están cacheados
            sql_pd = self.sql_pd(f'{self.ruta_datos}{self.tornillos_db}', self.tabla)
            self._df_cache = sql_pd.sql_to_df()  # Cargar y cachear el DataFrame
...
if __name__ == '__main__':
    ...
    # Crea una instancia de DatosISODIN13Ext
    datos = DatosISODIN13Ext()
